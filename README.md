...
Googleマップの店舗情報をもとに、レビューの信頼性とサクラリスクを推定するWebアプリです。
...
- リポジトリには [`.coderabbit.yaml`](.coderabbit.yaml) を配置済みです。
- GitHub App の CodeRabbit をこのリポジトリにインストールすると、`main` 向け PR で自動レビューが走ります。
- 必要に応じて PR コメントで手動実行できます。
  - `@coderabbitai review`
...
- AI分析: OpenRouter (`google/gemini-3.1-flash-lite-preview`)
- キャッシュ/制限: Cloudflare KV (24時間キャッシュ) + Durable Objects (原子的なレート制限、日次予算制御、メトリクス)
...
- IPハッシュベースのレート制限
- 日次予算上限による新規分析停止
- Durable Objects による原子的なカウンタ更新
- フロントエンドCSPとWorker JSONレスポンスのセキュリティヘッダー
...
`worker/wrangler.toml.example` を `worker/wrangler.toml` にコピーし、作成した `id` / `preview_id` を設定します。
`worker/wrangler.toml` はローカル設定ファイルとして `.gitignore` に含めています。
Durable Object `COUNTERS` のbindingと `AtomicCounter` migrationはexampleに含まれており、初回 `wrangler deploy` 時に適用されます。
...
curl -I https://bottakurichecker.uk
```
